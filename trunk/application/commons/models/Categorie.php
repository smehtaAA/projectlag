...
class Model_Categorie
{
    ...
    {
        if (null === $this->_table) {
            require_once APPLICATION_PATH_COMMONS . '/models/DbTable/Categorie.php';
	...
	{
		$table = $this->getTable();
		$select = $table->select()->from('categorie', array('idCategorie', 'titre', 'admin'))->where('visible=1')->order('ordre');
